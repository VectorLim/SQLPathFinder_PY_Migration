...
# - step_0016_sqlite_query (Line 1076): filters on a0.icmpcs
# - step_0043_sql_query (Line 1112): filters on f0.history_deleted_flag, f0.operation, f0.out_date, f0.owner, f4.history_deleted_flag, f4.unique_flag, f5.history_deleted_flag, f5.transaction, la.attribute_number, p.latest_version
# - step_0044_sql_query (Line 1115): filters on bams0.lot, bams0.operation, yeuchuan_a1_17680.tab
# - step_0048_sqlite_query (Line 1162): filters on FlagLot, NCO_Risk, a0.rowid, lot_1
...
class CsvIO:
    ...
    def sql_get_csv_list(self, path: 'str', column_ref: 'int | str', lead_in: 'str', chunk_size: 'int'=1000) -> 'str':
...
class PipelineContext:
    ...
    def run_query(self, sql: 'str', output: 'str', reader: 'Any', inputs: 'list[str | tuple[str, str]] | None'=None, header: 'list[str] | None'=None, crosstab: 'dict | None'=None, node: 'str | None'=None):
...
def step_0071_sqlite_query(ctx) -> None:
    ctx.run_query(sql="""
    SELECT DISTINCT 
     'BA' AS [MODULE]
    ...
    """, output='SPFMMSTMP.csv', reader=SqliteReader(), inputs=[('IPM_Data.csv', 'T0')], header=['MODULE', 'SIGNALTYPE', 'ALARMTYPE', 'EMAIL', 'HOLD_LOT', 'AUTO_CONTAIN', 'PRODGROUP3', 'ENTITY', 'IMPACT_LOT_VI'])

def step_0072_smart_append(ctx) -> None:
    ctx.smart_append.append('spfheaderfile.csv', 'SPFMMSTMP.csv')

def step_0073_fs_copy(ctx) -> None:
    ctx.fs_ops.rename(src='spfheaderfile.csv', dst='<TS>_' + ctx.macro.named('%USERNAME%') + '_output.pickle.csv')

def step_0074_fs_delete(ctx) -> None:
    ctx.fs_ops.delete(paths=['SPFMMSTMP.csv'])

def step_0075_fs_copy(ctx) -> None:
    ctx.fs_ops.copy(src=str(Path('.') / '*_output.pickle.csv'), dst=ctx.macro.named('MIPPATH'))

def step_0078_write_file(ctx) -> None:
...
def step_0079_external(ctx) -> None:
    ctx.external.run(argv=['update.bat'])

def step_0080_fs_delete(ctx) -> None:
...
def run() -> None:
    ...
    with ctx.macro.scope(ctx.csv_io.single_row('configsets.csv')):
        ...
        step_0046_rows_in_file(ctx)
        if Logger.condition('/PROMPT-TEXT=Step 5. Apply Conditional Logic', int(ctx.macro.named('ROWSINFILE')) > int('0')):
            step_0048_sqlite_query(ctx)
            step_0049_sqlite_query(ctx)
        step_0051_rows_in_file(ctx)
        if Logger.condition('/PROMPT-TEXT=Step 8-2. TRUE if signal is found', int(ctx.macro.named('SIGNAL')) > int('0')):
            if Logger.condition('/PROMPT-TEXT=Step 8-3. TRUE if CSR is enabled', ctx.macro.named('DCSR') == 'Y'):
                step_0054_sqlite_query(ctx)
                step_0055_fs_copy(ctx)
            step_0057_html_report(ctx)
            step_0058_html_report(ctx)
            step_0059_html_report(ctx)
            step_0060_rows_in_file(ctx)
            if Logger.condition('/PROMPT-TEXT=Step 8-8. TRUE if HISTERROR.txt is not found', int(ctx.macro.named('HISTERR')) <= int('0')):
                step_0062_smart_append(ctx)
                step_0063_fs_copy(ctx)
                step_0064_fs_copy(ctx)
        step_0067_rows_in_file(ctx)
        if Logger.condition('/PROMPT-TEXT=Step 9-2. TRUE if signal is found', int(ctx.macro.named('SIGNAL')) > int('0')):
            if Logger.condition('/PROMPT-TEXT=Step 9-3. TRUE if ICMPCS MMS Workflow is enabled', ctx.macro.named('DMWF') == 'Y'):
                step_0070_write_file(ctx)
                step_0071_sqlite_query(ctx)
                step_0072_smart_append(ctx)
                step_0073_fs_copy(ctx)
                step_0074_fs_delete(ctx)
                step_0075_fs_copy(ctx)
        step_0078_write_file(ctx)
        step_0079_external(ctx)
        step_0080_fs_delete(ctx)
# <vg2c:workflow:end>
