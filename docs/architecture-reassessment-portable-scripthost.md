...
- direct-runtime/architecture-reassessment-portable-scripthost-v3
...
## Additional controller/state characterization

The final reassessment also inspected the original control implementation rather than treating mutable task objects as a defect by default.

- `SPFTaskBase.__init__` explicitly assigns a new `childTasksList = []` to each task instance. The class-level declaration is therefore not, by itself, evidence that child task trees leak across task objects.
- `ForLoopTask` Version 2 deep-copies its child task tree for each iteration and substitutes the mature `spf-start-*`, `spf-end-*`, `spf-step-*`, and `spf-loop-ctr-*` token family.
- `SiteLoopTask` substitutes site tokens into a copied child tree and, on a child failure, resets `gMyAbort` and breaks the current site loop. Its console wording says "next Loop iteration", but the actual code uses `break`; compatibility must follow the actual code path unless product requirements explicitly change it.
- `RunLoopTask` owns chunking, per-chunk child execution, and its ErrTrap/global-abort interaction. That is semantic behavior, not merely a Windows transport.
- `BeginHPCTask` is recognized by the mature parser/task tree, but its execution path delegates to legacy SQLPF service clients. Current `vg2c_new` deliberately flattens BEGIN/END-HPC to a local scope. This is a clear keep-semantics/replace-or-retire-transport boundary rather than a reason to discard the original parser.

New Linux characterization tests verify that separate mutable task instances do not share their child list and that original `GetQuery` routes SITE-LOOP, RUN-LOOP, and BEGIN-HPC to the expected original task classes without invoking their host-specific transports.

## SPFGlobals state classification

| State class | Representative fields/behaviour | Migration treatment |
|---|---|---|
| CONSTANT / CONFIG | SQLFILE delimiter, token constants, date formats, library/version data, retry-code tables | Keep |
| PER-RUN MUTABLE | gMyAbort, gAnyLoop/gAnyLoopCtr, g_gVars, macro/script paths/data, command-line arguments, local/temp/exe paths, report counters | Keep under one-job-per-process isolation initially |
| PER-TASK THAT SHOULD NOT BE GLOBAL | A subset of loop/report/error bookkeeping is accessed globally even though it describes one execution | Do not refactor for style now; isolate by process and move only fields proven to block future requirements |
| PROCESS-GLOBAL INTEGRATION | global DB connection cache, logger/service settings, server/environment lookups | Replace or constrain at integration seams; do not share across concurrent jobs |
| WINDOWS STATE | service identity/.NET context, Win32-derived identity, Windows install/R/JMP/COM locations | Guard, replace transport, or retire when invoked |
| OBSOLETE / HISTORICAL | SQLPFaaS service transport where no longer required, generic DOS/BAT fallback, obsolete helper versions | Explicitly retire only after product confirmation; retain named parser recognition for clear errors |
