...
3.0.1.3 : vanatara : Update console statements. Support Pandas V2.2+ removed deprecated parameters in pandas.read_csv() 'keep_date_col', 'delim_whitespace', 'verbose'.
...
3.0.1.6 : vanatara : Add QueryClient (V1.0.0.6) support into SvcClient
3.0.1.7 : vanatara : Add CreateInGroupDataFile method to QueryClient Async class
"""
__version__ = "3.0.1.7"
#region imports
...
class QueryClient(object):
    ...
    def QueryName(self, value):
        ...
        """
        if value in ["", None]:
            errMsg = "Invalid 'QueryName' value"
        self.__QueryName = value  
    #endregion properties
    
    def _record_SPF(self):
            """
            log to SPFLogSvc
            """
            try:
                self.logger.debug(f"Start logSvc")
                headers = {'user-agent': 'SvcQueryClient/{0}'.format(__version__)}
                self._SQLPFSvcClient._record_SPF(logrec = self.LogRec, logEE = self.LogEE, req_headers = headers)
                self.logger.debug("Done logSvc")
            except Exception as err:
                self.logger.error(err)
                pass
    ...
    def __init__(self, 
                 QueryName: str
                ,SQLPFSvcEnv: str = "PROD"
                , SQLPF_CMD_Args_List:list = []
                , SPFInstance:str = ""
                , SPFLogLevel : str = "ERROR"
                , required_input_files_list_csv:str = ""
                , required_output_files_folder_list_csv:str = ""
                , optional_output_files_folder_list_csv:str = ""
                , display_svc_console_flag:bool = False
                , copy_log_files_flag:bool = False # True --> fetch the query execution logs from HPC server
                , continue_on_error:bool = False
                , begin_hpc_svc_ver:str = "2"
                ...
                , user_idsid:str = ""
                , svc_output_fetch_mode:str ="HTTP"
                , loggerName:str = "SvcQueryClient"
                , logFileName:str = "SvcQueryClient.log" # specify absolute path if location of log file needs to be created in a different location
                , async_status_check_sleep_time_seconds:int =30
                , SPF_CL_Args: dict={} ##command-line arguments e.g., {'CL_VAR_NAME' : 'VAR_VALUE'}. key should always begin with 'CL_'
                , SPF_LogSvc_Env: str = "SQLPF_LogSvc" #SPF logger Svc Environment Prod: 'SQLPF_LogSvc', Dev : 'SQLPF_LogSvc_CH_Dev'
                , copy_svc_console_flag: bool = False # True --> fetch the query execution console output from HPC server
                , displayConsoleMessages = False
                ) -> None:
        #create the logger
        self._get_logger(loggerName = loggerName, logFileName = logFileName, logLevel = SPFLogLevel)

        self.gDomains = ["amr.corp.intel.com", "ccr.corp.intel.com", "corp.intel.com", "gar.corp.intel.com", "ger.corp.intel.com"]
        self.QueryName = QueryName
        self.SQLPFSvcEnv = SQLPFSvcEnv
        self.SQLPF_CMD_Args_List = SQLPF_CMD_Args_List
        self.SPFInstance = SPFInstance
        self.SPFLogLevel = SPFLogLevel
        self.required_input_files_list_csv = required_input_files_list_csv
        self.required_output_files_folder_list_csv = required_output_files_folder_list_csv
        ...
        self.ClientAppName = ClientAppName
        self.SPFExe = SPFExe
        self.hpc_mem_limit_GB = hpc_mem_limit_GB
        self.hpc_svc_timeout_hours = hpc_svc_timeout_hours
        self.user_idsid = user_idsid
        self.svc_output_fetch_mode = svc_output_fetch_mode
        self.loggerName = loggerName
        self.async_status_check_sleep_time_seconds = async_status_check_sleep_time_seconds
        self.SPF_CL_Args = SPF_CL_Args
        self.SPF_LogSvc_Env = SPF_LogSvc_Env
        self.copy_svc_console_flag = copy_svc_console_flag
        self.displayConsoleMessages = displayConsoleMessages
        self.logCallingScript = Path(sys.argv[0]).name
        self.LogRec = [f"QueryClientAsync_{__version__}", f"{self.logCallingScript}"] #build logging record
        # self.logger.debug("Initialized")
        self.LogEE = "QCASYNC_API"
        self.queryFile_temp = None # temp local path for the .spfsql file downloaded over http
        self.list_of_files_to_delete = []
        self.__init_SvcClient()
    #END : def __init__

    def __init_SvcClient(self):
        """
        Initialize svcClient instance
        """
        self.logger.debug(f"  Environment : '{self.SQLPFSvcEnv}'")
        SQLPFSvcEnv_lookup = {"PROD" : "SQLPFAAS_PROD_NS"
                              ,"PROD_N1" : "SQLPFaaS_PROD_CH_N1_V2_NS"
                              ,"PROD_N2" : "SQLPFaaS_PROD_CH_N2_V2_NS"
                              ,"DEVCH" : "SQLPFaaS_DEVCH_V2_NS"
                              ,"LOCAL" : ""}
        queryFileName = self._getQuery()
        try:
            _SQLPFSvcEnv = SQLPFSvcEnv_lookup[self.getMatchingValFrmList(self.SQLPFSvcEnv, list(SQLPFSvcEnv_lookup.keys()))]
        except KeyError as keyErr:
            errMsg  =f"SQLPathfinder service environment '{self.SQLPFSvcEnv}' not found"
        except Exception as err:
            raise
        try:
            self._SQLPFSvcClient = SQLPFSvcClient(SQLPFSvcEnv = _SQLPFSvcEnv
                                                            , SQLPF_CMD_Args_List = self.SQLPF_CMD_Args_List
                                                            , SPFSQL_File = queryFileName
                                                            , SPFInstance = self.SPFInstance
                                                            , SPFLogLevel = self.SPFLogLevel
                                                            , required_input_files_list_csv = self.required_input_files_list_csv
                                                            , required_output_files_folder_list_csv = self.required_output_files_folder_list_csv
                                                            , optional_output_files_folder_list_csv = self.optional_output_files_folder_list_csv
                                                            ...
                                                            , user_idsid = self.user_idsid
                                                            , svc_output_fetch_mode = self.svc_output_fetch_mode
                                                            # , loggerName = self.loggerName
                                                            , async_status_check_sleep_time_seconds = self.async_status_check_sleep_time_seconds
                                                            , SPF_CL_Args = self.SPF_CL_Args
                                                            , SPF_LogSvc_Env = self.SPF_LogSvc_Env
                                                            , copy_svc_console_flag = self.copy_svc_console_flag
                                                            )
        except Exception as err:
            self.logger.error(err)
            raise
        self.logger.debug("svcClient Initialized...")

    def getMatchingValFrmList(self, valToCheck, inValueList):
        #locals        
        self.logger.debug("valToCheck: {0}".format(valToCheck))
        self.logger.debug("inValueList: {0}".format(inValueList))
        MatchingVal = None
        matchvalToCheck = re.compile("^{0}$".format(valToCheck.replace("|", "\\|").replace("[", "\[").replace("]", "\]")), re.IGNORECASE)
        ...
        if itemCount == 0 :
            ...
            self.logger.debug("MatchingVal: {0}".format(MatchingVal))
            return MatchingVal
    ...
    def _getQuery(self):
        ...
        """
        #region nested function
        def get_config_csv() -> bytes:
            """
            get the SQLPFaaS NS config.csv file using GOTO or static URL
            """
            ENV_NS_CONFIG_GOTO_Lookup = {'DEVCH'    : f"HTTPS://GOTO.INTEL.COM/SQLPFaaS_NS_CONFIG_DEVCH"
                                        ,'PROD'     : f"HTTPS://GOTO.INTEL.COM/SQLPFaaS_NS_CONFIG_PROD"
                                        ,'PROD_N1'  : f"HTTPS://GOTO.INTEL.COM/SQLPFaaS_NS_CONFIG_PROD_N1"
                                        ,'PROD_N2'  : f"HTTPS://GOTO.INTEL.COM/SQLPFaaS_NS_CONFIG_PROD_N2"
                                        ,'PROD_N3'  : f"HTTPS://GOTO.INTEL.COM/SQLPFaaS_NS_CONFIG_PROD_N3"
                                        ,'PROD_N4'  : f"HTTPS://GOTO.INTEL.COM/SQLPFaaS_NS_CONFIG_PROD_N4"
                                        ,'PROD_N5'  : f"HTTPS://GOTO.INTEL.COM/SQLPFaaS_NS_CONFIG_PROD_N5"}

            ENV_NS_CONFIG_DIRECT_Lookup = {'DEVCH'    : f"https://dtdvdwspfwb1.amr.corp.intel.com/SQLPFaaS_NS_Config/SQLPFaaS_Query_Config.csv"
                                          ,'PROD'     : f"https://atdspwspfbwlb1.ch.intel.com/SQLPFaaS_NS_Config/SQLPFaaS_Query_Config.csv"
                                          ,'PROD_N1'  : f"https://atdspwspfbwbn1.amr.corp.intel.com/SQLPFaaS_NS_Config/SQLPFaaS_Query_Config.csv"
                                          ,'PROD_N2'  : f"https://atdspwspfbwbn2.amr.corp.intel.com/SQLPFaaS_NS_Config/SQLPFaaS_Query_Config.csv"
                                          ,'PROD_N3'  : f"https://atdspwspfbwbn3.amr.corp.intel.com/SQLPFaaS_NS_Config/SQLPFaaS_Query_Config.csv"
                                          ,'PROD_N4'  : f"https://atdspwspfbwbn4.amr.corp.intel.com/SQLPFaaS_NS_Config/SQLPFaaS_Query_Config.csv"
                                          ,'PROD_N5'  : f"https://atdspwspfbwbn5.amr.corp.intel.com/SQLPFaaS_NS_Config/SQLPFaaS_Query_Config.csv"}
            config_csv_url = None
            try:
                goto_url = ENV_NS_CONFIG_GOTO_Lookup[self.SQLPFSvcEnv]

                r = requests.get(goto_url, verify = False) 
                config_csv_url = r.url
                if r.url.lower() == goto_url.lower():
                    errMSg = f"Error : SQLPFSvcEnv '{self.SQLPFSvcEnv}' not found in GOTO.Intel.com"
                    raise Exception(errMSg)
            except KeyError as keyErr:
                self.logger.debug(f"WARN : SQLPFSvcEnv '{self.SQLPFSvcEnv}' not found in GOTO, tyring DIRECT lookup")
                try:
                    config_csv_url = ENV_NS_CONFIG_DIRECT_Lookup[self.SQLPFSvcEnv]
                except KeyError as keyErr2:
                    errMSg = f"Error : SQLPFSvcEnv '{self.SQLPFSvcEnv}' not found in direct lookup"
                    raise Exception(errMSg)
            ...
            self.logger.debug(f"config_csv_url : {config_csv_url}")
            r = requests.get(config_csv_url, headers=headers, verify=False, auth=HTTPKerberosAuth(),  stream=True)
            self.logger.debug(f"config_csv_url Req Status : {r.status_code}")
            _req_content = r.content
            r.close()
            return config_csv_url, BytesIO(_req_content)
        #END : get_config_csv
        #endregion nested function

        #main method
        headers  = {"Cache-Control" : "no-cache, no-store, must-revalidate", "Pragma": "no-cache", "Expires" : "0"}
        config_csv_url, config_csv_content = get_config_csv()
        queryConfigDF = pandas.read_csv(config_csv_content, sep=",")
        queryRow = queryConfigDF[queryConfigDF['QueryName'] == self.QueryName]
        
        if len(queryRow) == 0:
            errMsg = f"QueryName : '{self.QueryName}' not found"
            raise Exception(errMsg)
        elif len(queryRow) > 1:
            errMsg = f"More than one query data found for QueryName : '{self.QueryName}'"
            raise Exception(errMsg)
        
        self.LogRec.append(f"{self.QueryName}")

        queryFile = queryRow['QueryPath'].values[0]
        self.logger.debug(f"queryFile : {queryFile}")
        queryFile_pth = Path(queryFile)
        if queryFile_pth.exists() is False:
            errMsg = f"Query file '{queryFile}' for Query Name : '{self.QueryName}' not found"
            self.logger.debug("SPFSQL file not found based on network path. Retry using webpath")
            try:
                self.queryFile_temp = f"{self.QueryName}.spfsql"
                config_csv_url_prsd = urlparse(config_csv_url)
                spfsql_url = spfsql_url = urlunparse(config_csv_url_prsd._replace(path=f"/SQLPFaaS_NS_Config/SPFSQL/{self.queryFile_temp}"))
                self.logger.debug(f"spfsql_url : {spfsql_url}")
                response = requests.get(spfsql_url, headers=headers, verify=False, auth=HTTPKerberosAuth(),  stream=True)
                
                self.logger.debug(f"writing spfsql to file : {self.queryFile_temp}")
                with open(self.queryFile_temp, "wb") as contentWrtr:
                    contentWrtr.write(response.content)
                self.list_of_files_to_delete.append(self.queryFile_temp)
                self.logger.debug(f"completed writing spfsql to file : {self.queryFile_temp}")
                self.logger.debug(f"  Query Name : '{self.QueryName}'")
                self.logger.debug(f"  Query File : '{self.queryFile_temp}'")
                return self.queryFile_temp
            except Exception as err:
                self.logger.error(err)
                raise Exception(errMsg)
        self.logger.debug(f"  Query Name : '{self.QueryName}'")
        self.logger.debug(f"  Query File : '{queryFile}'")
        return queryFile
    #END : def __getQuery        
    ...
    def Execute(self):
        ...
        Execute the query
        """    
        self.logger.debug(f"Starting SQLPathfinder HPC Async Query Client V{__version__}")
        self.LogRec.append("ex")
        
        try:
            self.logger.debug("Execute Start...")
            
            # self._SQLPFSvcClient.displayConsMsg = False
            self._SQLPFSvcClient.Execute()
            self.logger.debug("Execute Done...")
        except Exception as err:
            raise
        finally:
            self._record_SPF()
    ...
    def cleanUp(self):
        try:
            FileToDelete_pathObj:Path
            for file_item in self.list_of_files_to_delete:
                try:
                    if type(file_item) is str:
                        FileToDelete_pathObj = Path(file_item)
                    else:
                        FileToDelete_pathObj = file_item
                    FileToDelete_pathObj.unlink()
                    self.logger.debug(f"Deleted file : {file_item}")
                except Exception as err:
                    self.logger.warn(f"Error while deleting : {file_item}")        
        except Exception as err:
            self.logger.warn(f"Error while cleanUp : {err}")
    ...
    def CreateInGroupDataFile(self, InGroupData: Dict, InGroupDataFileName:str, DeleteInGroupDataFile: bool = True):
        ...
        3. DeleteInGroupDataFile : bool : True - delete file specified in 'InGroupDataFileName' after execution. False - Don't delete the file
        """
        self._SQLPFSvcClient.CreateInGroupDataFile(InGroupData = InGroupData, InGroupDataFileName=InGroupDataFileName, DeleteInGroupDataFile=DeleteInGroupDataFile)
    #END : def CreateInGroupDataFile
#END : class QueryClient

class QueryClientSync(QueryClient):
    """
    ...
    def __init__(self, QueryName: str
                ...
                , user_idsid:str = "" # IDSID of the actual user. Note: this is just for logging/tracking...no security action prformed based on this value
                , svc_output_fetch_mode:str ="HTTP" #default is get output over HTTP
                , loggerName:str = "SvcQueryClient" # pass name of the logger that is already created by the calling client... 
                , convert_output_to_json:bool = False # sync svc only : true -> convert final output to json format
                , required_output_file: str = "" # sync svc only : 'file name' of final output file of spfsql execution that should be read and sent back in Svc response
                , hpc_svc_timeout_minutes = 10 #HPC Sync svc timeout in minutes 
                , SPF_CL_Args: dict={} #command-line arguments e.g., {'CL_VAR_NAME' : 'VAR_VALUE'}. key should always begin with 'CL_'
        ...
        # super().__init__(QueryName, SQLPFSvcEnv, SQLPF_CMD_Args_List, SPFInstance, SPFLogLevel, required_input_files_list_csv, required_output_files_folder_list_csv, optional_output_files_folder_list_csv, display_svc_console_flag, copy_log_files_flag, continue_on_error, begin_hpc_svc_ver, ClientAppName, SPFExe, hpc_mem_limit_GB, hpc_svc_timeout_hours, user_idsid, svc_output_fetch_mode, loggerName, async_status_check_sleep_time_seconds, SPF_CL_Args, SPF_LogSvc_Env, copy_svc_console_flag, QueryClientLogLevel)
        super().__init__(QueryName = QueryName
                     , SQLPFSvcEnv=SQLPFSvcEnv
                    #  , SQLPFSvcURL=SQLPFSvcURL
                     , SQLPF_CMD_Args_List=SQLPF_CMD_Args_List
                    #  , SPFSQL_File=SPFSQL_File
                     , SPFLogLevel=SPFLogLevel
        ...
        self.hpc_svc_timeout_minutes = hpc_svc_timeout_minutes
        self.convert_output_to_json = convert_output_to_json
        self.LogRec = [f"QueryClientSync_{__version__}", f"{self.logCallingScript}"] #build logging record
        self.LogEE = "QCSYNC_API"
        self.ExecuteCycleDone = False
        self.GetDF = False
        self.__init_SvcClient()
        #endregion - set class level variables
    ...
    def __init_SvcClient(self):
        ...
        """
        self.logger.debug(f"  Environment : '{self.SQLPFSvcEnv}'")
        SQLPFSvcEnv_lookup = {"PROD" : "SQLPFaaS_PROD_SYNC_NS"
                              ,"PROD_N1" : "SQLPFaaS_PRODSYNC_CH_N1_V2_NS"
                              ,"PROD_N2" : "SQLPFaaS_PRODSYNC_CH_N2_V2_NS"
                              ,"DEVCH" : "SQLPFaaS_DEVCH_V2_Sync_NS"
                              ,"LOCAL" : ""}
        queryFileName = self._getQuery()
        try:
            _SQLPFSvcEnv = SQLPFSvcEnv_lookup[self.getMatchingValFrmList(self.SQLPFSvcEnv, list(SQLPFSvcEnv_lookup.keys()))]
        ...
        try:
            self._SQLPFSvcClient = SQLPFSvcClientSync(SQLPFSvcEnv = _SQLPFSvcEnv
                                                            , SQLPF_CMD_Args_List = self.SQLPF_CMD_Args_List
                                                            , SPFSQL_File = queryFileName
                                                            , SPFInstance = self.SPFInstance
                                                            , SPFLogLevel = self.SPFLogLevel
                                                            ...
                                                            , svc_output_fetch_mode = self.svc_output_fetch_mode
                                                            # , loggerName = self.loggerName
                                                            , convert_output_to_json = self.convert_output_to_json
                                                            , required_output_file = self.required_output_file
                                                            , hpc_svc_timeout_minutes = self.hpc_svc_timeout_minutes
                                                            , SPF_CL_Args = self.SPF_CL_Args
                                                            , SPF_LogSvc_Env = self.SPF_LogSvc_Env
                                                            , copy_svc_console_flag = self.copy_svc_console_flag
                                                            , copy_log_files_flag = self.copy_log_files_flag
    ...
    def Execute(self):
        ...
        Execute the query
        """ 
        self.logger.debug(f"Starting SQLPathfinder HPC Sync Query Client V{__version__}")
        self.LogRec.append("ex")
        self.logger.debug("Execute Start...")
        try:
            # self._SQLPFSvcClient.displayConsMsg = False
            if self.GetDF is True:
                self.logger.debug("get_df() called...Skip Execute...")
                return
            self._SQLPFSvcClient.Execute()
            self.ExecuteCycleDone = True
            self.logger.debug("Execute Done...")
    ...
    def get_df(self, delete_svc_output_file=True):
        ...
        Note: loads the entire service response output into a DF. Large files might cause issues
        """
        self.GetDF = True # set this flag to True to indicate...get_df() is invoked and execute will be internally invoked. _record_spf() should be called only once
        self.LogRec.append("df")
        try:
            if self.ExecuteCycleDone is False:
                self.Execute()
            return self._SQLPFSvcClient.get_df(delete_svc_output_file=delete_svc_output_file)
        finally:
            self.cleanUp()
            self._record_SPF()
    #END : get_df
    ...
    def CreateInGroupDataFile(self, InGroupData: Dict, InGroupDataFileName:str, DeleteInGroupDataFile: bool = True):
        ...
        self._SQLPFSvcClient.CreateInGroupDataFile(InGroupData = InGroupData, InGroupDataFileName=InGroupDataFileName, DeleteInGroupDataFile=DeleteInGroupDataFile)
    #END : def CreateInGroupDataFile
#END : class QueryClientSync        

if __name__ == "__main__":
    print("Executing in command line mode : Start\n")
    def test_svcClient_ASync():
        ...
        _SQLPFSvcClient = SQLPFSvcClient(SQLPFSvcEnv = SQLPFSvcEnv
    ...
    def test_QueryClientAsync():
        """
        """   
        QueryName = "Query_test_SQLPFHPC"
        _SQLPFSVcEnv = "DEVCH" 
        _SPFLogLevel = "DEBUG"
        SPF_CL_Args = {'CL_MARS_DB' : 'ATD.[A43_Prod_0.].MARS',
                   'CL_DAYS_BACK' : 3}
        _QueryClient = QueryClient(QueryName = QueryName
                                    , SQLPFSvcEnv = _SQLPFSVcEnv
                                    , SPFLogLevel="DEBUG"
                                    )
        _QueryClient.Execute()

    def test_QueryClientSync():
        """
        """
        QueryName = "Query_test_SQLPFHPC"
        _SQLPFSVcEnv = "DEVCH"
        _SPFLogLevel = "DEBUG"
        _required_output_file = "midas_output.tab"
        SPF_CL_Args = {'CL_MARS_DB' : 'ATD.[A43_Prod_0.].MARS',
                   'CL_DAYS_BACK' : 3}
        _QueryClientSync = QueryClientSync(QueryName = QueryName
                                # , SQLPFSvcEnv = _SQLPFSVcEnv
                                , required_output_file = _required_output_file
                                , SPF_CL_Args=SPF_CL_Args
                                # , SPFLogLevel="DEBUG"
                                )
        _QueryClientSync.CreateInGroupDataFile({'params' : ["%"]}, 'LOT_LIST.CSV')
        df = _QueryClientSync.get_df()
        print(df)
        
    test_QueryClientAsync()
    # test_QueryClientSync()
