...
def test_script_short_flat_scope_and_no_runtime_refs(FIXTURES: Path) -> None:
    ...
    assert sum(len(b.runtime_macro_refs) for b in program.blocks) == 0


def test_sql_script_detects_sql_get_csv_list_call(FIXTURES: Path) -> None:
    program = _resolve_fixture(FIXTURES, "sql_script.txt")
    calls = [call for b in program.blocks for call in b.sql_macro_calls]
    assert len(calls) == 1
    assert calls[0].csv_path.lower().endswith("yeuchuan_a0_29397.tab")
    assert calls[0].column_ref == "lot"


def test_calendar_ref_producer_before_consumer(FIXTURES: Path) -> None:
    for fixture_name in ("script_another.txt", "script_from_vietnam.txt"):
        program = _resolve_fixture(FIXTURES, fixture_name)
        producer_idx = program.csv_producers.get("calendar_ref.csv")
        assert producer_idx is not None
        consumer_idxs = program.csv_consumers.get("calendar_ref.csv", ())
        if consumer_idxs:
            assert any(producer_idx < idx for idx in consumer_idxs)
...
def test_actual_script_scope_and_macro_signals(FIXTURES: Path) -> None:
    ...
    bound_names = {
        ...
        for ref in block.runtime_macro_refs
        if ref.frame_id != -1
    }
    assert required.issubset(bound_names)

    calls = [call for block in program.blocks for call in block.sql_macro_calls]
    assert len(calls) >= 4
    ...
    ctime_infos = [
        ...
        if d.code == "unknown-csv-producer" and "ctime.csv" in d.message.lower()
    ]
    assert len(ctime_infos) == 1
