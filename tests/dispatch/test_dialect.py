...
from vg2c.dispatch.dialect import SQL_BEARING_KINDS, derive_from_signals, resolve_dialect
...
@pytest.mark.parametrize(
    "kind",
    [Kind.WRITE_FILE, Kind.UTILITY, Kind.MACRO_CONTROL, Kind.HTML_REPORT, Kind.UNKNOWN, Kind.MALFORMED],
)
...
def test_derive_from_signals_oasys_by_node() -> None:
    assert derive_from_signals(node="KM.OASYS", engine="VA", oledb="SQLPlus") == "oracle_oasys"


def test_derive_from_signals_mars_by_node_suffix() -> None:
    assert derive_from_signals(node="KM.[A15_PROD_21.].MARS", engine="VA", oledb="SQLPlus") == "oracle_mars"


def test_derive_from_signals_mars_placeholder_node() -> None:
    assert derive_from_signals(node="<<<MARS>>>", engine="VA", oledb="SQLPlus") == "oracle_mars"


def test_derive_from_signals_aries_by_node() -> None:
    assert derive_from_signals(node="<<<ARIES>>>", engine="VA", oledb="SQLPlus") == "oracle_aries"
