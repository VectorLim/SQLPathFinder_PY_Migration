...
_SQL_BEARING = {Kind.MARS_READ, Kind.OASYS_READ, Kind.ARIES_READ, Kind.SQLITE_QUERY}
...
def test_full_pipeline_runs_without_exception(FIXTURES: Path, fixture_name: str) -> None:
    program = _run_pipeline(FIXTURES, fixture_name)
    assert isinstance(program.dispatched, tuple)


@pytest.mark.parametrize("fixture_name", FIXTURE_NAMES)
def test_dispatched_count_equals_sql_bearing_blocks(FIXTURES: Path, fixture_name: str) -> None:
    """dispatched tuple has exactly one entry per SQL-bearing block."""
    program = _run_pipeline(FIXTURES, fixture_name, config=DispatchConfig(oasys_schema="SCHEMA"))
    sql_block_count = sum(
...
def test_sql_script_oasys_unset_emits_error(FIXTURES: Path) -> None:
    ...
    assert any(d.code == "dispatch-oasys-schema-unset" and d.severity == "error" for d in program.diagnostics)
...
def test_sql_script_oasys_schema_substituted(FIXTURES: Path) -> None:
    ...
    program = _run_pipeline(FIXTURES, "sql_script.txt", config=DispatchConfig(oasys_schema="OASYS_OWN"))
...
def test_actual_script_record_names_present(FIXTURES: Path) -> None:
    program = _run_pipeline(FIXTURES, "actual_script.txt", config=DispatchConfig())
    record_names = {d.reader_target.record_name for d in program.dispatched if d.reader_target.record_name}
