...
def test_detects_write_file_and_table_consumer() -> None:
    ...
    assert any(p.csv_path == "foo.csv" and p.producer_kind == "write-file" for p in program.producers)
    assert any(c.csv_path == "foo.csv" and c.consumer_kind == "table" for c in program.consumers)
    assert any(e.csv_path == "foo.csv" and e.producer is not None for e in program.edges)
...
def test_sqlite_block_can_be_producer_and_consumer() -> None:
    program = _analyze_blocks(
        [
            ...
            _block(1, Kind.SQLITE_QUERY, {"ENGINE": "SQLite", "TABLE": "a.csv", "CSV": "b.csv"}),
        ]
    )
    assert any(p.csv_path == "b.csv" and p.producer_kind == "sqlite-query" for p in program.producers)
...
def test_external_utility_candidate_softens_missing_producer() -> None:
    program = _analyze_blocks(
        [
            _block(0, Kind.UTILITY, utilities='@EXEDIR@\\SPFCopy.bat "source.csv" "." "N"'),
            _block(1, Kind.MACRO_CONTROL, utilities='{ROWS-IN-FILE} "source.csv" "CNT" "N"'),
        ]
    )
    assert any(d.code == "dataflow-likely-external-producer" for d in program.diagnostics)
...
def test_branch_exclusive_producers_emit_info() -> None:
    ...
    assert any(d.code == "dataflow-branch-exclusive-producers" for d in program.diagnostics)


def test_unused_producer_emits_info() -> None:
    program = _analyze_blocks([_block(0, Kind.WRITE_FILE, {"WRITE-FILE": "Y", "CSV": "orphan.csv"})])
...
def test_table_comma_split_patch_from_stage2() -> None:
    ...
    paths = [c.csv_path for c in program.consumers if c.block_index == 2 and c.consumer_kind == "table"]
