...
def test_mutable_task_instances_keep_child_lists_per_instance() -> None:
    _with_extracted_runtime()
    module = importlib.import_module("SPFLib.SPFSQL3")
    source = _block('/UTILITIES={FOR-LOOP} "0" "1" "1" "x" "N"')

    first = module.ForLoopTask(source, 0)
    second = module.ForLoopTask(source, 1)
    first.childTasksList.append(object())

    assert first.childTasksList is not second.childTasksList
    assert second.childTasksList == []


@pytest.mark.parametrize(
    ("utility_line", "expected_task"),
    [
        ('/UTILITIES={SITE-LOOP} "A.MARS,B.MARS"', "SiteLoopTask"),
        ('/UTILITIES={RUN-LOOP} "input.csv" "chunk.csv" "10" "Y"', "RunLoopTask"),
        ('/UTILITIES={BEGIN-HPC} "SQLPFSVC_DEVCH"', "BeginHPCTask"),
    ],
)
def test_original_getquery_routes_controller_tasks_on_linux(
    tmp_path, utility_line: str, expected_task: str
) -> None:
    _with_extracted_runtime()
    module = importlib.import_module("SPFLib.SPFSQL3")
    manager = module.SPFManager()
    manager.gCommandLineArguments = [
        "scripthost-controller-routing-probe",
        f"/MYLOCAL={tmp_path}",
        f"/EXEDIR={tmp_path}",
    ]

    task = manager.GetQuery(str(tmp_path), _block(utility_line), 0, False)

    assert type(task).__name__ == expected_task
