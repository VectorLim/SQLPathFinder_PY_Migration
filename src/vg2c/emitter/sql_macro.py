"""SQL-exclusive macro helpers.

This module contains runtime expansion logic for SQLPathFinder SQL macros such
as ``SQL_Get_CSV_List(...)``. These are separate from placeholder-based
SQLPathFinder macros (``<<<NAME>>>`` / ``<<>>``), which remain in
:mod:`vg2c.emitter.macro`.
"""
