...
_RESOLVER_MANIFEST_BASE: tuple[tuple[str, str, str, str, str, str], ...] = (
    ...
    (r"@EXEDIR@\CSVToXML.va", "CSVToXMLTask", "csv_to_xml", "DIRECT_PORT", "csv/xml", "format parity"),
    (r"@EXEDIR@\SPFDelete.bat", "SPFDeleteTask", "delete_file", "AMENDED_PORT", "filesystem", "list/wildcard delete"),
    (r"@EXEDIR@\SQLPathFinder_Email.va", "EmailTask", "email", "REWRITE", "DataSyncX", "mail contract"),
    (r"@EXEDIR@\ImportExcel.va", "ImportExcelTask", "import_excel", "AMENDED_PORT", "excel", "sheet import"),
    (r"@EXEDIR@\LoadExcel.va", "LoadExcelTask", "load_excel", "AMENDED_PORT", "excel", "sheet load"),
    (r"@EXEDIR@\XLSToCSV.va", "XLSToCSVTask", "xls_to_csv", "AMENDED_PORT", "excel/csv", "conversion"),
    (r"@EXEDIR@\SetFileRO.va", "SetFileROTask", "set_file_read_only", "AMENDED_PORT", "filesystem", "permissions"),
    (r"@EXEDIR@\Get_Aqua.va", "GetAquaTask", "get_aqua", "REWRITE", "network", "current integration"),
    (r"@EXEDIR@\Get_TCA.va", "GetTCATask", "get_tca", "REWRITE", "network", "current integration"),
    (r"U->Hadoop-Delete:Y@EXEDIR@\Hadoop-Delete.va", "HadoopDeleteTask", "hadoop_delete", "REWRITE", "external", "current backend"),
    (r"U->Hadoop-Load:Y@EXEDIR@\Hadoop-Load.va", "HadoopLoadTask", "hadoop_load", "REWRITE", "external", "current backend"),
    (r"U->Prompt-Input:Y@EXEDIR@\PROMPT-INPUT.va", "PromptInputTask", "prompt_input", "REWRITE", "interactive", "input behavior"),
    (r"@EXEDIR@\LoadJMP.va", "LoadJMPTask", "load_jmp", "REWRITE", "JMP/Windows", "product decision"),
    (r"@EXEDIR@\Run_Python_Script.va", "RunPythonScriptTask", "run_python", "AMENDED_PORT", "python", "process contract"),
    ...
    (r"@EXEDIR@\PublishToSP.va", "PublishToSPTask", "publish_sharepoint", "REWRITE", "SharePoint", "integration"),
    (r"@EXEDIR@\SmartAppend.va", "SmartAppendTask", "smart_append", "DIRECT_PORT", "csv", "append parity"),
    (r"@EXEDIR@\SPFRename.va", "SPFRenameTask", "rename_file", "AMENDED_PORT", "filesystem", "rename parity"),
    (r"@EXEDIR@\SQLiteDelete.va", "SQLiteDeleteTask", "sqlite_delete", "DIRECT_PORT", "sqlite", "delete parity"),
    (r"@EXEDIR@\SQLite-Load.va", "SQLiteLoadTask", "sqlite_load", "DIRECT_PORT", "sqlite", "load parity"),
    ...
    (r"@EXEDIR@\SPFDISTRIBUTE.VA", "SPFDistributeTask", "distribute", "AMENDED_PORT", "filesystem", "distribution"),
    (r"@EXEDIR@\WaitInterval.va", "WaitIntervalTask", "wait_interval", "DIRECT_PORT", "time", "wait parity"),
    (r"@EXEDIR@\WaitFile.va", "WaitFileTask", "wait_file", "AMENDED_PORT", "filesystem", "poll parity"),
    (r"@EXEDIR@\Get_Web_Text.exe", "GetWebTextTask", "get_web_text", "REWRITE", "http", "http parity"),
    ...
    (r"@EXEDIR@\GetHelpCSV.va", "GetHelpCSVTask", "get_help_csv", "AMENDED_PORT", "csv", "help output"),
    (r"MONGODB-EXPORT.VA", "MongoExportTask", "mongo_export", "REWRITE", "DataSyncX", "mongo parity"),
    (r"U->MongoDB-Import:Y@EXEDIR@\MongoDB-Import.va", "MongoImportTask", "mongo_import", "REWRITE", "DataSyncX", "mongo parity"),
    (r"U->MongoDB-Extract:Y@EXEDIR@\MongoDB-Extract.va", "MongoExtractTask", "mongo_extract", "REWRITE", "DataSyncX", "mongo parity"),
    (r"@EXEDIR@\VA_Join_By_Name.va", "SPFJoinTask", "join_by_name", "DIRECT_PORT", "csv", "join parity"),
    ...
    ("{DATE-OF-FILE}", "DateOfFileTask", "date_of_file", "DIRECT_PORT", "filesystem", "value parity"),
    ("{GET-SITE-TIME}", "GetSiteTimeTask", "get_site_time", "AMENDED_PORT", "time", "site time"),
    ("{GET-SITE-TIME-FILE}", "GetSiteTimeTask", "get_site_time_file", "AMENDED_PORT", "time/filesystem", "site time file"),
    ...
    ("{UPDATE-TIME-FILE}", "UpdateTimeFileTask", "update_time_file", "DIRECT_PORT", "time/filesystem", "file parity"),
    ("{VALUE-IN-FILE}", "ValueInFileTask", "value_in_file", "DIRECT_PORT", "filesystem", "value parity"),
    ("{GET-FILES}", "GetFilesTask", "get_files", "AMENDED_PORT", "filesystem", "glob parity"),
    ("{SHAREPOINT-DELETE}", "DeleteSPTask", "sharepoint_delete", "REWRITE", "SharePoint", "integration"),
    ("{FILE-COMPARE}", "FileCompareTask", "file_compare", "DIRECT_PORT", "filesystem", "comparison"),
    ...
    ("{ENCRYPT-SPFSQL-FILE}", "EncryptSPFSQLFileTask", "encrypt_spfsql", "REWRITE", "crypto", "security decision"),
    ("{ENCRYPT-CONFIG-FILE}", "EncryptConfigFileTask", "encrypt_config", "REWRITE", "crypto", "security decision"),
 )
...
_RESOLVER_MANIFEST_BASE += (
    ...
    ("{START-MACRO}", "StartMacroTask", "macro", "AMENDED_PORT", "csv/runtime", "first-row/nesting"),
    ...
    ("{BEGIN-HPC}", "BeginHPCTask", "hpc_scope", "FLATTEN_TRANSPORT", "runtime", "local child execution"),
    ...
    ("HTML-LAYOUT", "HTMLLayoutTask", "report.html_layout", "AMENDED_PORT", "html", "report parity"),
    ("HTML-TAB-LAYOUT", "HTMLTaborMenuLayoutTask", "report.html_tab_layout", "AMENDED_PORT", "html", "report parity"),
    ("HTML-MENU-LAYOUT", "HTMLTaborMenuLayoutTask", "report.html_menu_layout", "AMENDED_PORT", "html", "report parity"),
    ("HTML-GNUPLOT", "HTMLGNUPlotTask", "report.gnuplot", "REWRITE", "plotting", "report parity"),
    ("HTML-GNUPLOT-SHOW", "HTMLGNUPlotTask", "report.gnuplot_show", "REWRITE", "plotting", "report parity"),
    ...
    ("HTML-DELETE", "HTMLDeleteTask", "report.delete", "AMENDED_PORT", "filesystem/html", "report parity"),
    ...
    ("HTML-PYPLOT-SHOW", "HTMLPyPlotTask", "report.pyplot_show", "AMENDED_PORT", "plotting", "report parity"),
    ("NQ_CBSQL_ORACLE_Task", "nqCBSQLTask", "query.cbsql_oracle", "REWRITE", "DataSyncX", "routing/query parity"),
    ("NQ_UBER_Task", "nqUberTask", "query.uber", "REWRITE", "DataSyncX", "routing/query parity"),
    ("NQ_SQLSERVER_TASK", "nqSQLServerTask", "query.sqlserver", "REWRITE", "DataSyncX", "routing/query parity"),
    ("NQ_HADOOP_TASK", "nqHadoopTask", "query.hadoop", "REWRITE", "DataSyncX", "routing/query parity"),
    ("NQ_SQLITE_TASK", "nqSQLiteTask", "query.sqlite", "AMENDED_PORT", "sqlite", "routing/query parity"),
    ("NQ_CB_TASK", "nqCBTask", "query.cb", "REWRITE", "DataSyncX", "routing/query parity"),
    ("NQ_ORACLE_TASK", "nqOracleTask", "query.oracle", "REWRITE", "DataSyncX", "routing/query parity"),
    ("NQ_TERADATA_TASK", "nqTeradataTask", "query.teradata", "REWRITE", "DataSyncX", "routing/query parity"),
    ("NQ_TEXT_TASK", "nqTextTask", "query.text", "AMENDED_PORT", "filesystem", "routing/query parity"),
    ("NQ_MONGO_TASK", "nqMongoTask", "query.mongo", "REWRITE", "DataSyncX", "routing/query parity"),
    ("NQ_HADOOP_IMPALA_TASK", "nqHadoopImpalaTask", "query.impala", "REWRITE", "DataSyncX", "routing/query parity"),
    ("NQ_MSOLAP_TASK", "nqMSOLAPTask", "query.msolap", "REWRITE", "DataSyncX", "routing/query parity"),
    ("NQ_MYSQL_TASK", "nqMySQLTask", "query.mysql", "REWRITE", "DataSyncX", "routing/query parity"),
    ("NQ_SAPHANA_TASK", "nqSAPHanaTask", "query.saphana", "REWRITE", "DataSyncX", "routing/query parity"),
    ("NQ_DENODO_TASK", "nqDenodoTask", "query.denodo", "REWRITE", "DataSyncX", "routing/query parity"),
    ("NQ_PYSCRIPTDRIVER_TASK", "nqPyScriptDriverTask", "query.pyscript", "AMENDED_PORT", "python", "routing/query parity"),
    ("NQ_PYSCRIPTDRIVERIMPORT_TASK", "nqPyScriptDriverImportTask", "query.pyscript_import", "AMENDED_PORT", "python", "routing/query parity"),
    ("NQ_DUCKDB_TASK", "nqDuckDBTask", "query.duckdb", "AMENDED_PORT", "duckdb", "routing/query parity"),
    ("NQ_SNOWFLAKE_TASK", "nqSnowFlakeTask", "query.snowflake", "REWRITE", "DataSyncX", "routing/query parity"),
    ("NQ_PostgreSQL_TASK", "nqPostgreSQLTask", "query.postgresql", "REWRITE", "DataSyncX", "routing/query parity"),
...
def parse_utility_arguments(value: str, span: SourceSpan | None = None) -> tuple[str, ...]:
    ...
    """
    try:
        reader = csv.reader(StringIO(value.strip()), delimiter=" ", skipinitialspace=True, quotechar='"')
...
def _resolve(
    ...
    if utility_value is not None:
        ...
        if manifest is None:
            raise _error(span, f"Unsupported /UTILITIES command {alias!r}; shell fallback is disabled.")
        target = manifest[2]
...
def _resolve_query(options: dict[str, str]) -> str:
    ...
    if re.search(r"(^\$CB\$\@?)", user, re.IGNORECASE) or (re.search(r"\.yas", site, re.IGNORECASE) and not user.strip()):
...
def _parse_sequence(
    ...
    while position < len(flat):
        ...
        if command.kind is CommandKind.IF:
            children, position, marker = _parse_sequence(flat, position + 1, frozenset({"else", "end_if"}))
            if marker is None:
                raise _error(command.span, "Unclosed {IF-THEN}; expected {END-IF}.")
            else_children: list[Command] = []
            if marker == "else":
                else_children, position, marker = _parse_sequence(flat, position + 1, frozenset({"end_if"}))
                if marker != "end_if":
                    raise _error(command.span, "Unclosed {ELSE}; expected {END-IF}.")
            result.append(replace(command, children=tuple(children), else_children=tuple(else_children)))
            position += 1
            continue
        ...
        if expected_end is not None:
            children, position, marker = _parse_sequence(flat, position + 1, frozenset({expected_end}))
            if marker != expected_end:
                raise _error(command.span, f"Unclosed {command.command_type}; expected {expected_end}.")
