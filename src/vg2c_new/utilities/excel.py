...
class XlsToCsvUtility(Utility):
    def apply(self, command: Command, state: RuntimeState) -> None:
        ...
        if len(args) < 2:
            raise ValueError("XLSToCSV requires Excel input and CSV output.")
        source, output = _path(command, state, args[0]), _path(command, state, args[1])
        sheet: int | str = 0
        if len(args) > 2 and args[2].strip():
            sheet = int(args[2]) - 1 if args[2].strip().isdigit() else args[2]
        skiprows = int(args[3]) if len(args) > 3 and args[3].isdigit() else 0
        skipfooter = int(args[4]) if len(args) > 4 and args[4].isdigit() else 0
        frame = pd.read_excel(
            source, sheet_name=sheet, skiprows=skiprows, skipfooter=skipfooter, dtype=str
        ).fillna("")
        frame.columns = [str(c).replace("\n", " ") for c in frame.columns]
        CsvUtility.write_dataframe(frame, output)
...
class LoadExcelUtility(Utility):
    def apply(self, command: Command, state: RuntimeState) -> None:
        ...
        try:
            frame = CsvUtility.read_dataframe(_path(command, state, args[0]))
            output = _path(command, state, args[1])
...
class ImportExcelUtility(Utility):
    def apply(self, command: Command, state: RuntimeState) -> None:
        ...
        args = [state.substitute(v) for v in command.arguments]
        if len(args) < 4:
            raise ValueError("ImportExcel requires template, result workbook, CSV and worksheet.")
        source_book, result_book, csv_name, worksheet = args[:4]
        ...
        if vb_proc:
            raise RuntimeError(
                "VBA procedure execution is intentionally not supported in the portable Excel runtime."
            )
        try:
            from openpyxl import Workbook, load_workbook

            source = _path(command, state, source_book)
            result = _path(command, state, result_book)
            ...
            frame = CsvUtility.read_dataframe(_path(command, state, csv_name))
