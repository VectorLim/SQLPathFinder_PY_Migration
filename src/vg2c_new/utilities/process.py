...
class RunPythonUtility(Utility):
    def apply(self, command: Command, state: RuntimeState) -> None:
        ...
        if not args:
            raise ValueError("Run_Python_Script requires a script path.")
        if len(args) > 4 and args[4].strip().upper() in {"PYTHON-V3-OLD", "PYTHON-V3-NEXT"}:
            raise RuntimeError(
                f"Historical Python selector {args[4].strip().upper()!r} is intentionally removed."
            )
...
class PyScriptUtility(Utility):
    def apply(self, command: Command, state: RuntimeState) -> None:
        ...
        args = [state.substitute(v) for v in command.arguments]
        if not args:
            raise ValueError("{PYSCRIPT} requires a Python script file.")
        script = _path(command, state, args[0])
        if not script.exists():
            raise FileNotFoundError(script)
        done = subprocess.run(
            [sys.executable, str(script), *args[1:]], cwd=state.working_directory, check=False
        )
        if done.returncode not in {0, 2}:
            raise RuntimeError(f"Python script exited with code {done.returncode}.")
...
class RunRUtility(Utility):
    def apply(self, command: Command, state: RuntimeState) -> None:
        ...
        args = [state.substitute(v) for v in command.arguments]
        if not args:
            raise ValueError("Run_R_Script requires a script path.")
