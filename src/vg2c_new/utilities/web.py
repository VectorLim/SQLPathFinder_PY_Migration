...
class GetWebTextUtility(Utility):
    ...
    def apply(self, command: Command, state: RuntimeState) -> None:
        ...
        if len(args) < 2:
            raise ValueError("Get_Web_Text requires URL and output file.")
        url, output = args[:2]
        version = args[2].strip().upper().replace(" ", "") if len(args) > 2 else "VERSION2"
        if version not in {"", "VERSION2"}:
            raise RuntimeError("Historical Get-Web-Text V1 is intentionally removed.")
        auth = args[3].strip().upper() if len(args) > 3 else "N"
        no_abort = args[4].strip().upper() in {"Y", "YES", "TRUE", "1"} if len(args) > 4 else False
        timeout = int(args[5]) if len(args) > 5 and args[5].isdigit() else 60
        try:
            if auth in {"Y", "YES", "TRUE", "1"}:
                raise RuntimeError(
                    "Integrated Windows authentication is not portable; configure an approved authenticated requests session."
                )
