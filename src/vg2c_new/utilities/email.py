...
class EmailUtility(Utility):
    def __init__(self, send_msg: Callable[..., Any] | None):
    ...
    def apply(self, command: Command, state: RuntimeState) -> None:
        ...
        if self._send_msg is None:
            raise RuntimeError("DataSyncX send_msg is unavailable in this environment.")
        args = [state.substitute(v) for v in command.arguments]
        if len(args) < 3:
            raise ValueError("Email requires attachment list, recipients and subject.")
        attachments = _split(args[0])
        recipients = _split(args[1])
        subject = args[2]
        body_value = args[3] if len(args) > 3 else ""
        ...
        cc = _split(args[4]) if len(args) > 4 else []
        bcc = _split(args[5]) if len(args) > 5 else []
        resolved = [str(_path(command, state, v)) for v in attachments]
        self._send_msg(
            to=recipients, subject=subject, body=body, attachments=resolved, cc=cc, bcc=bcc
        )
