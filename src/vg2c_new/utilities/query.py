...
class OracleQueryUtility(Utility):
    def __init__(
        self,
        *,
        mars_reader: Any = None,
        aries_reader: Any = None,
        oasys_reader: Any = None,
        oracle_reader: Any = None,
    ):
        self._mars_reader = mars_reader
        self._aries_reader = aries_reader
        self._oasys_reader = oasys_reader
        self._oracle_reader = oracle_reader
    ...
    def apply(self, command: Command, state: RuntimeState) -> None:
        ...
        """
        node = state.substitute(command.option("NODE", "") or "")
        sql = state.substitute(command.body)
        reader, sql = self._route(node, sql)
        if reader is None:
            raise RuntimeError(f"No DataSyncX reader is available for Oracle node {node!r}.")
        ...
        if headers:
            ...
            if requested and len(requested) == len(frame.columns):
                frame.columns = requested
        output = command.option("CSV")
        if output:
            CsvUtility.write_dataframe(frame, _path(command, state, state.substitute(output)))

    def _route(self, node: str, sql: str) -> tuple[Any, str]:
        upper = node.upper()
        if "MARS" in upper:
            return self._mars_reader, _MARS_MISSING_DOT.sub("@[]@.", sql)
        if "ARIES" in upper:
            return self._aries_reader, sql
        if "OASYS" in upper:
            return self._oasys_reader, sql.replace("@OASYSSCHEMA@", "")
    ...
    def _site(node: str, state: RuntimeState) -> str:
        ...
        if not value or value.startswith("<<<"):
            value = state.lookup("NODE") or state.environment_value("VG2C_DEFAULT_NODE") or ""
        if "." in value:
...
class GetSiteTimeUtility(Utility):
    def __init__(
        ...
        *,
        ...
        self._query = OracleQueryUtility(
            mars_reader=mars_reader,
            aries_reader=aries_reader,
            oasys_reader=oasys_reader,
    ...
    def apply(self, command: Command, state: RuntimeState) -> None:
        ...
        if not args or not args[0].strip():
            raise ValueError("{GET-SITE-TIME} requires a site or GMT.")
        site = args[0].strip()
        if site.upper() == "GMT":
            value = datetime.now(UTC).strftime("%Y-%m-%d %H:%M:%S")
        else:
            reader, query = self._query._route(
                site,
                "SELECT To_Char(sysdate,'YYYY-MM-DD hh24:mi:ss') AS last_update_date FROM dual WHERE rownum <= 1",
            )
            if reader is None:
                raise RuntimeError(f"No DataSyncX reader is available for site {site!r}.")
            frame = _as_frame(reader.read(site=OracleQueryUtility._site(site, state), query=query))
            if frame.empty:
                raise RuntimeError(f"No site time returned for {site!r}.")
            value = str(frame.iloc[0, 0])
        state.set_global("SPF_SITE_TIME", value)
        state.set_global("SPF_JOB_DT", value)
...
class PlatformGapUtility(Utility):
    ...
    def apply(self, command: Command, state: RuntimeState) -> None:
        raise RuntimeError(f"Current-platform gap for {self._target}: {self._reason}")
