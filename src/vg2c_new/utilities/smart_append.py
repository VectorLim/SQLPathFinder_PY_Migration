...
class SmartAppendUtility(Utility):
    def apply(self, command: Command, state: RuntimeState) -> None:
        ...
        if len(args) < 6:
            raise ValueError(
                "SmartAppend V4 requires old file, new file, delete column/value, update column and sort-headers flag."
            )
        old, new = _path(command, state, args[0]), _path(command, state, args[1])
        delete_col, delete_value, key_col, sort_headers = args[2:6]
        abort_old = _yn(args[6]) if len(args) > 6 else False
        update_file = args[7].strip() if len(args) > 7 else ""
        version = args[8].strip().upper().replace(" ", "") if len(args) > 8 else "VERSION4"
        default_new = args[10] if len(args) > 10 and args[10].strip() else ""
        upper = _yn(args[12]) if len(args) > 12 else False
        if version and version != "VERSION4":
            raise RuntimeError(
                "Historical SmartAppend versions are intentionally removed; use VERSION4 semantics."
            )
        if not new.exists():
            return
        ...
        if new_frame.empty:
            return
        if abort_old and not old.exists():
            raise FileNotFoundError(f"Old SmartAppend file not found: {old}")
        old_frame = self._read(old, upper) if old.exists() else pd.DataFrame()
        delete_col = delete_col.upper() if upper else delete_col
        key_col = key_col.upper() if upper else key_col
        if delete_col and delete_value:
            cutoff = self._delete_cutoff(delete_value, state)
            new_frame = self._apply_delete(new_frame, delete_col, cutoff)
            if not old_frame.empty:
                old_frame = self._apply_delete(old_frame, delete_col, cutoff)
        if key_col and not old_frame.empty and not new_frame.empty:
            new_key = self._column(new_frame, key_col, "Update")
            old_key = self._column(old_frame, key_col, "Update")
            keys = set(new_frame[new_key].astype(str))
            old_frame = old_frame[~old_frame[old_key].astype(str).isin(keys)]
        columns = self._union_columns(old_frame, new_frame, sort_headers)
        fill = _default_value(default_new)
        old_frame = (
            old_frame.reindex(columns=columns, fill_value="")
            if not old_frame.empty
            else pd.DataFrame(columns=columns)
        )
        new_frame = new_frame.reindex(columns=columns, fill_value=fill)
        result = pd.concat([new_frame, old_frame], ignore_index=True, sort=False).fillna("")
        tmp = old.with_name(f".{old.name}.smartappend.tmp")
        CsvUtility.write_dataframe(result, tmp)
        old.parent.mkdir(parents=True, exist_ok=True)
        tmp.replace(old)
        if update_file:
            from vg2c_new.model import Command as C
            from vg2c_new.model import CommandKind, SourceSpan
            ...
            synthetic = C(
                ...
                command.raw,
                (update_file,),
                SourceSpan(command.span.file, command.span.start_line, command.span.end_line),
            )
    ...
    def _read(self, path: Path, upper: bool) -> pd.DataFrame:
        ...
        if any(not str(c).strip() for c in frame.columns):
            raise ValueError(f"{path} contains an empty column header.")
        return frame.fillna("")

    def _apply_delete(self, frame: pd.DataFrame, column: str, cutoff: str) -> pd.DataFrame:
    ...
    def _delete_cutoff(self, value: str, state: RuntimeState) -> str:
        ...
        if not m:
            return text
        amount = int(float(m.group(2)))
        units = m.group(3).lower()
        if m.group(1).lower() == "today_gmt":
            raw = state.lookup("SPF_GW_DT")
            base = (
            ...
            raw = state.lookup("SPF_JOB_DT") or state.lookup("SPF_SITE_TIME")
            base = datetime.strptime(raw, "%Y-%m-%d %H:%M:%S") if raw else datetime.now()
        if units.startswith("day"):
            base = base.replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(days=amount)
        else:
