...
async def upload_workspace_files(
    ...
    if len(files) != len(paths):
        raise HTTPException(status_code=400, detail="Each uploaded file must include a relative path.")
    manager = get_workspace_manager(request)
    workspace = get_workspace(request)
...
def download_workspace_archive(request: Request) -> FileResponse:
    manager = get_workspace_manager(request)
    workspace = get_workspace(request)
    handle = tempfile.NamedTemporaryFile(prefix="vg2c-workspace-", suffix=".zip", delete=False)
    archive_path = Path(handle.name)
    handle.close()
    try:
        with zipfile.ZipFile(archive_path, "w", compression=zipfile.ZIP_DEFLATED) as archive:
            for item in manager.list_files(workspace):
                archive.write(manager.resolve_file(workspace, item.path), arcname=item.path)
