...
export function ScriptTree({ document, projection, search, expandedScopes, selectedId, revealVersion, revealFocus, onSelect, onToggleScope }: Props) {
  const treeRef = useRef<HTMLUListElement>(null)
  useEffect(() => {
    const row = [...(treeRef.current?.querySelectorAll<HTMLButtonElement>('[data-tree-id]') ?? [])].find((item) => item.dataset.treeId === selectedId)
    if (!row) return
  ...
  const effects = projection?.documents.find((item) => item.document_id === document.id)?.effects ?? document.effects
  const visibleIds = query ? searchVisibility(document, query, effects) : null
  const selectedStep = document.steps.find((step) => step.operations.some((operation) => operation.id === selectedId))
  if (visibleIds && selectedId) {
    visibleIds.add(selectedStep?.id ?? selectedId)
    for (const ancestor of ancestorScopeIds(document, selectedId)) visibleIds.add(ancestor)
  }
  ...
  const firstRoot = roots.find((item) => !visibleIds || visibleIds.has(item.id))
  const focusableId = selectedVisible ? selectedId : firstRoot?.node_kind === 'step' ? firstRoot.operations[0]?.id ?? firstRoot.id : firstRoot?.id ?? null
  ...
  function render(items: ScriptItem[], depth: number): ReactNode {
    return items.map((item) => {
      ...
      const targetId = item.node_kind === 'step' ? item.operations[0]?.id ?? item.id : item.id
      const selected = selectedId === targetId
      ...
      return <li className={`tree-node${selected ? ' is-selected' : ''}${hasError ? ' has-error' : ''}`} key={item.id} role="none">
        <button type="button" className="tree-row" role="treeitem" aria-level={depth + 1} aria-selected={selected} aria-expanded={isScope ? expanded : undefined} aria-invalid={hasError || undefined} tabIndex={targetId === focusableId ? 0 : -1} data-tree-item data-tree-id={targetId} data-parent-id={item.parent_scope_id ?? ''} onClick={() => { onSelect(targetId); if (isScope) onToggleScope(item.id) }} onKeyDown={(event) => handleTreeKey(event, item, expanded, onToggleScope)}>
          ...
          <TreeLabel item={item} effects={effects} />
        ...
        {item.node_kind === 'step' && item.operations.slice(1).map((operation) => <button key={operation.id} type="button" className="tree-row tree-operation" role="treeitem" aria-level={depth + 1} aria-selected={operation.id === selectedId} tabIndex={operation.id === focusableId ? 0 : -1} data-tree-item data-tree-id={operation.id} data-parent-id={item.parent_scope_id ?? ''} onClick={() => onSelect(operation.id)} onKeyDown={(event) => handleTreeKey(event, item, false, onToggleScope)}><span aria-hidden="true">{item.block_index + 1}</span><span>{operation.utility.title}: {operation.utility.method}</span></button>)}
  ...
  return <ul ref={treeRef} className="script-tree" role="tree" aria-label="Script operations">{render(roots, 0)}{query && visibleIds?.size === 0 && <li className="empty-copy">No operations match “{search.trim()}”.</li>}</ul>
}

function TreeLabel({ item, effects }: { item: ScriptItem; effects: FileEffectView[] }) {
  ...
  const label = formatOperationLabel(item, item.operations[0], effects)
...
function searchVisibility(document: DocumentView, query: string, effects: FileEffectView[]): Set<string> {
  const result = new Set<string>()
  for (const item of [...document.scopes, ...document.steps]) {
    const text = item.node_kind === 'step' ? [item.description, item.display_label, ...item.operations.flatMap((operation) => { const label = formatOperationLabel(item, operation, effects); return [label.primary, label.secondary] })].filter(Boolean).join(' ') : item.label
...
function handleTreeKey(event: KeyboardEvent<HTMLButtonElement>, item: ScriptItem, expanded: boolean, toggle: (id: string, expanded?: boolean) => void) { const items = visibleTreeItems(event.currentTarget.closest('[role="tree"]')); const index = items.indexOf(event.currentTarget); if (event.key === 'ArrowDown' && index < items.length - 1) { event.preventDefault(); items[index + 1]?.focus() } else if (event.key === 'ArrowUp' && index > 0) { event.preventDefault(); items[index - 1]?.focus() } else if (event.key === 'Home') { event.preventDefault(); items[0]?.focus() } else if (event.key === 'End') { event.preventDefault(); items.at(-1)?.focus() } else if (event.key === 'ArrowRight' && item.node_kind !== 'step') { event.preventDefault(); if (!expanded) toggle(item.id, true); else items.find((candidate) => candidate.dataset.parentId === item.id)?.focus() } else if (event.key === 'ArrowLeft') { if (item.node_kind !== 'step' && expanded) { event.preventDefault(); toggle(item.id, false); return } const parentId = event.currentTarget.dataset.parentId; if (parentId) { event.preventDefault(); items.find((candidate) => candidate.dataset.treeId === parentId)?.focus() } } }
function visibleTreeItems(root: Element | null): HTMLButtonElement[] { return [...(root?.querySelectorAll<HTMLButtonElement>('[data-tree-item]') ?? [])].filter((element) => element.offsetParent !== null && getComputedStyle(element).visibility !== 'hidden') }
